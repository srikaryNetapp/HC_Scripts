...
#   This script is part of a 6 part script to perform Health check of the infrastructure components(Storage, Compute, Network)
#   This is the main function and other scripts are imported into it.
# 
#                                              This is a NetApp Internal script 
...
def sms():
  ...
  script_path = "C:\\HC_Scripts\\sms.ps1"
...
def html_header_templat(current_time, date, storage_hc_body, vmware_hc_body, switch_hc_body, css_gen, jq_script):
    ...
    <style> {css} </style>
    <body>
        <h1> <strong> Allianz - Infrastructure Health Check Report - {date} </strong></h1>
...
if __name__ == "__main__":
    ...
    #   Import storage list
    #####################################
    print("Importing storage cluster details")
    ...
    solidfire_clusters = clusters["SOLIDFIRE"]

    #####################################
    #   Import switch list
    #####################################
    # print("Importing Switch  details")
    ...
    #####################################
    try:
        print("Fetching VMware Heatch report")
        out = vmware_cli()
        output = vmware_cli() #out.decode("utf-8")
    except:
        output = "<TD bgcolor=#FA8074> Unable to Fetch VMWare Health Status </TD>"
    ...
    for cluster in ontap_clusters:
        cluster_name, cluster_version, cluster_status, deg_node_name = storagesc.get_cluster_status(
            cluster)
        cluster_names += [cluster_name]
        nodes = storagesc.get_cluster_nodes(cluster)
        cluster_nodes[cluster_name] = nodes
    css_gen, hover_var_list, trg_var_list = css_genarator(
        cluster_names, cluster_nodes)
    jq_script = script_generator(hover_var_list, trg_var_list)
    try:
        print("Fetching Storage Health report")
        storage_hc_body = storagesc.ontap_report_table(
            ontap_clusters, storgrid_clusters, eseries_clusters, solidfire_clusters)
    except:
        storage_hc_body = "<TD bgcolor=#FA8074> Unable to Fetch Storage Health Status </TD>"
    vmware_hc_body = output
    # try:
    #     print("Fetching Switch Health Report")
    #     switch_hc_body = switchhc.cumulus_table(
    #         cumulus_switches, cisco_switches)
    # except:
    switch_hc_body = "<TD bgcolor=#FA8074> Unable to Fetch Switch Health Status </TD>"
    ...
    with io.open(filelocation, 'w', encoding="utf-8") as file:
        file.write(htmlout)
    #subprocess.call("./sms.py")
    # subprocess.call("./sms_test.py")
    print(sms())

end_time = time.time()
print(f"Script took {end_time-start_time:.2f} seconds to complete")
